...
/** Keys kept when purging local cache — session, launch gates, and cloud auth tokens must survive. */
const CLEAR_CACHE_PRESERVE_EXACT = new Set([
  'isLoggedIn',
  'currentUserId',
  'app_settings',
  'users',
...
function shouldPreserveOnCacheClear(key: string): boolean {
  if (CLEAR_CACHE_PRESERVE_EXACT.has(key)) return true;
  // Supabase Auth (persistSession) — wiping these logs the user out via onAuthStateChanged.
  if (key.startsWith('sb-')) return true;
  // Failover / provider flags (instacollab_auth_backend, instacollab_supabase_unhealthy, …)
  if (key.startsWith('instacollab_')) return true;
  return false;
}
...
export class DbCore {
    ...
    protected save(key: string, data: unknown) {
    ...
    protected load<T>(key: string, defaultData: T): T {
    ...
    public clearCache() {
      ...
      Object.keys(this.cache).forEach((key) => {
        if (shouldPreserveOnCacheClear(key)) {
          preservedCache[key] = this.cache[key];
        }
      });
      this.cache = preservedCache;

      // Preserve selected localStorage keys (including Supabase sb-* session).
