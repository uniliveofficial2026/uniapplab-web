...
async function hydrateCloudAppStateForUser(userId: string): Promise<HydrateResult> {
  if (isSupabaseConfigured() && (await hasSupabaseSessionForUser(userId))) {
    ...
    realtimeUnsub = subscribeSupabaseUserAppState(userId, (payload) => {
...
export async function startCloudAppStateRealtime(userId: string): Promise<void> {
  ...
  try {
    hydrateResult = await hydrateCloudAppStateForUser(userId);
  } catch (err) {
