...
export function CloudAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const queueApplySupabase = useCallback((next: Session | null) => {
    const generation = ++applyGeneration.current;
    window.setTimeout(() => {
      if (generation !== applyGeneration.current) return;
      void applySupabaseSessionToLocalDb(next).catch((err) => {
