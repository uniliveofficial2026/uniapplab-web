           ...
           >
              ...
              {Array.isArray(msg.replyToMany) && msg.replyToMany.length > 0 && (
               ...
               {Array.isArray(msg.forwardedBundle) && msg.forwardedBundle.length > 0 ? (
                 ...
                 />
               ) : (typeof msg.text === 'string' && (msg.text.includes('instacollab.app/p/') || msg.text.includes('instacollab.app/r/') || msg.text.includes('instacollab.app/s/'))) ? (
                 <div 
                   className="flex flex-col gap-2.5 cursor-pointer max-w-[280px] sm:max-w-[320px] bg-black/25 hover:bg-black/40 p-3 rounded-2xl border border-white/5 transition-all shadow-sm active:scale-[0.98]" 
                   onClick={(e) => {
                     e.stopPropagation();
                     if (typeof msg.text === 'string') handleSharedItemClick(msg.text);
                   }}
                 >
                 {/* Header of the shared card */}
                 {(() => {
                   let user: { avatarUrl?: string; username?: string } | undefined;
                   let typeText: string;
                   if (msg.text.includes('/p/')) {
                     const match = msg.text.match(/\/p\/([^?\s]+)/);
                     const post = match ? db.posts.find((p: Post) => p.id === match[1]) : null;
                     user = post?.user;
                     typeText = post?.videoUrl ? 'Video Post' : post?.audioUrl ? 'Audio Post' : 'Post';
                   } else if (msg.text.includes('/r/')) {
                     const match = msg.text.match(/\/r\/([^?\s]+)/);
                     const reel = match ? db.reels.find((r: Reel) => r.id === match[1]) : null;
                     user = reel?.user;
                     typeText = 'Reel';
                   } else {
                     const match = msg.text.match(/\/s\/([^?\s]+)/);
                     user = db.users.find((u: User) => match && u.username === match[1]);
                     typeText = 'Story';
                   }

                   return user ? (
                     <div className="flex items-center justify-between border-b border-white/5 pb-2 w-full gap-4">
                       <div className="flex items-center gap-2">
                         ...
                         <div className="flex flex-col leading-none">
                           <span className="font-bold text-xs hover:underline text-white drop-shadow-sm truncate">@{user.username}</span>
                           <span className="text-[9px] text-zinc-400 font-medium tracking-wide leading-none mt-0.5">{typeText}</span>
                         </div>
                       </div>
                       <div className="text-[9px] font-bold uppercase tracking-wider text-white bg-white/10 px-2.5 py-1 rounded-[6px] border border-white/10 shrink-0 select-none">
                         View
                       </div>
                     </div>
                   ) : null;
                 ...
                 {msg.text.replaceAll(/https?:\/\/instacollab\.app\/\S+/g, '').replace(/[:Shared\s]+/g, '').trim() && (
                   <span className="text-[13px] sm:text-[14px] leading-relaxed font-semibold text-zinc-100 drop-shadow-sm px-1">
                 ...
                 <div className="rounded-xl overflow-hidden bg-black/40 aspect-video w-full relative shadow-inner border border-zinc-900 group">
                     {(() => {
                      if (msg.text.includes('/p/')) {
                        ...
                        return (
                          <div className="w-full h-full relative bg-black">
                            {reel?.videoUrl && (
                              <ChatInlineVideo
                                src={reel.videoUrl}
                                poster="https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=400&fit=crop"
                                onError={handleMediaError}
                              />
                            )}
                            <div className="absolute inset-0 flex items-center justify-center bg-black/40 flex-col gap-1 text-white pointer-events-none">
                        ...
                        const match = msg.text.match(/\/s\/([^?\s]+)/);
                        const storyUser = db.users.find((u: User) => match && u.username?.toLowerCase() === match[1]?.toLowerCase());
                        ...
                        return (
                          <div className="w-full h-full relative bg-zinc-950" style={{ backgroundImage: `url(${segment?.url || 'https://images.unsplash.com/photo-1517836357463-d25dfeac3438?w=400'})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                            {segment?.isVideo ? (
                              <ChatInlineVideo
                                src={segment?.url || ''}
                                poster="https://images.unsplash.com/photo-1517836357463-d25dfeac3438?w=400"
                                onError={handleMediaError}
                              />
                            ) : (
