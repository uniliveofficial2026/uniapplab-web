...
export function MusicDiscPlayer({ url, name = 'Audio Track', color = 'primary' }: MusicDiscPlayerProps) {
    ...
    <div className={`flex flex-col gap-3 p-3 sm:p-4 rounded-3xl border bg-white/5 backdrop-blur-xl shadow-2xl transition-all w-full max-w-[400px] ${
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          ...
          {/* Scrubber Handle */}
          <motion.div
            className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-white shadow-lg border-2 ${color === 'primary' ? 'border-primary' : 'border-secondary'} scale-0 group-hover:scale-100 ${isDragging ? 'scale-100' : ''} transition-transform z-10`}
