...
export function GoogleDocsTab() {
  ...
  const fetchDocs = async () => {
    ...
    try {
      ...
      setDocs(mapped.length > 0 ? mapped : seedDocs);
    } catch (e) {
  ...
  const handleSaveDocument = async () => {
    ...
    if (googleAccessToken) {
      try {
        // To update document contents dynamically in Google Docs, we issue requests to replace existing indices
        // For reliability, we replace the contents of the document. Under normal circumstances, this is a batchUpdate:
        const updateBody = {
          requests: [
            {
              insertText: {
        ...
        if (res.ok) {
          ...
          throw new Error('Google batch validation service rejected payload.');
        }
      } catch (err: any) {
          ...
          <div className="flex-grow flex flex-col h-[550px]">
            ...
            <div className="p-4 border-b border-border bg-secondary/5 flex justify-between items-center gap-4 bg-card">
              <div className="flex-1 min-w-0">
                ...
                />
                <p className="text-[10px] text-muted-foreground font-semibold flex items-center gap-1 mt-1"><History className="w-3 h-3" /> Synced locally • Auto saving enabled</p>
              </div>
