...
const pairs = SECRETS.filter(([, value]) => value).map(
...
if (fly.status !== 0) {
  ...
  if (/payment information|credit card|buy credit/i.test(errText)) {
    console.error(
      "[fly] Billing required — add a card at https://fly.io/dashboard/billing",
    );
    console.error(
      "[fly] Fly is optional; API runs on Vercel after: pnpm run vercel:fix-root",
    );
    process.exit(2);
  }
