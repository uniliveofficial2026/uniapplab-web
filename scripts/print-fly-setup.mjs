...
console.log('   Linux:  curl -L https://fly.io/install.sh | sh');
console.log('   Then restart terminal (install.sh adds flyctl to PATH)');
console.log('');
console.log('2) Login + create app (first time only)');
console.log('   flyctl auth login');
console.log(`   flyctl apps create ${flyApp} --org personal   # or your org slug`);
console.log('');
...
console.log(`   flyctl certs add api.uniapplab.com -a ${flyApp}`);
