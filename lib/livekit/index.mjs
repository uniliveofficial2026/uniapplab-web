...
export function partyRoomName(roomId) {
  return `ic-party-${roomId}`;
}

export function chatCallRoomName(threadId, callKind = 'audio') {
  const kind = callKind === 'video' ? 'video' : 'audio';
  return `ic-chat-call-${kind}-${threadId}`;
}
...
export async function createLiveKitToken({
  ...
  const publish = hidden ? false : (canPublish ?? role === 'host');
  // Hidden platform watchers must not surface admin identity/name in the room roster.
  const watchIdentity = hidden
    ? `aw_${String(identity).replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 36) || 'watch'}`
    : identity;
  const at = new AccessToken(apiKey, apiSecret, {
    identity: watchIdentity,
    name: hidden ? ' ' : name || identity,
    ttl: '6h',
  });
  at.addGrant({
    roomJoin: true,
    ...
    canPublish: publish,
    canSubscribe: true,
    // Hidden admin watchers must not broadcast data or appear in the room roster.
