...
export function startStudioServer(options = {}) {
  ...
  /** @type {{ actorId: string, organizationId?: string } | null} */
  let studioSession = options.session || null;
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/api/auth/login' && req.method === 'POST') {
        ...
        const actorId = body.actorId || `dev_${Date.now().toString(36)}`;
        const org = cloud.createOrganization({
          name: body.organizationName || `${actorId}-org`,
          ownerActorId: actorId,
        });
        studioSession = { actorId, organizationId: org.organizationId };
        observe.log('info', 'studio.login', { actorId, organizationId: org.organizationId });
        return sendJson(res, 200, {
          ok: true,
          actorId,
          organization: org,
          note: 'developer identity for Studio control plane (not end-user app auth)',
        });
      }

      if (url.pathname === '/api/auth/whoami' && req.method === 'GET') {
        return sendJson(res, 200, { ok: true, session: studioSession });
      }

      if (url.pathname === '/api/cloud/projects' && req.method === 'POST') {
        if (!studioSession?.organizationId) {
          return sendJson(res, 401, { ok: false, error: 'studio_auth_required' });
        }
        const body = await readBody(req);
        const result = cloud.createProject({
          organizationId: studioSession.organizationId,
          name: body.name || 'studio-project',
          actorId: studioSession.actorId,
        });
