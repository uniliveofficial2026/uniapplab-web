...
export function createUniLiveMcpServer(options = {}) {
  ...
  const tools = {
    ...
    async cloud_create_organization({ name, ownerActorId }) {
      const auth = authorize('project:write', null);
      if (!auth.ok) return deny(auth.reason);
      const { createUniLiveCloud } = await import('@unilives/cloud');
      const cloud = options.cloud || createUniLiveCloud({ controlPlane });
      const org = cloud.createOrganization({ name, ownerActorId: ownerActorId || 'mcp' });
      return { ok: true, organization: org };
    },

    async cloud_create_project({ organizationId, name, actorId }) {
      const auth = authorize('project:write', null);
      if (!auth.ok) return deny(auth.reason);
      const { createUniLiveCloud } = await import('@unilives/cloud');
      const cloud = options.cloud || createUniLiveCloud({ controlPlane });
    ...
    async marketplace_list({ query } = {}) {
      const auth = authorize('project:read', null);
      if (!auth.ok) return deny(auth.reason);
      const { createMarketplaceRegistry } = await import('@unilives/marketplace');
      const registryMp = createMarketplaceRegistry();
      return { ok: true, items: query ? registryMp.search(query) : registryMp.list() };
    },

    async ai_plan({ requirement, projectId }) {
      const auth = authorize('project:write', projectId || null);
      if (!auth.ok) return deny(auth.reason);
      const { createPlanner } = await import('@unilives/ai-builder');
      const planner = createPlanner();
      const result = await planner.buildFromRequirement({
        projectId: projectId || 'project_mcp',
        requirement: requirement || 'Create a basic page',
      });
      return { ok: true, ...result };
    },
