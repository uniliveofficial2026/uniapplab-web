...
import { loadCloudSnapshot, createSupabaseCloudDurable } from './durable-supabase.mjs';
...
/**
 * UniLive Cloud control plane MVP.
 * Default: in-memory (tests/local).
 * Production: pass `initialSnapshot` + `durable` (Supabase write-through).
 */
export function createUniLiveCloud(options = {}) {
  ...
  if (snap) {
    ...
    for (const row of snap.audit || []) audit.push(row);
...
export { loadCloudSnapshot, createSupabaseCloudDurable } from './durable-supabase.mjs';

/**
 * Production helper: hydrate from Supabase then write-through.
 */
export async function createDurableUniLiveCloud({ supabase, controlPlane } = {}) {
  ...
  return createUniLiveCloud({ controlPlane, initialSnapshot, durable });
}
