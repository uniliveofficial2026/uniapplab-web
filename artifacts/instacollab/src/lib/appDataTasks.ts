...
export const APP_DATA_TASKS: Record<TaskCategory, AppDataTask[]> = {
  ...
  launch: [
    ...
    { action: 'Profile setup', dbMethod: 'commitUserProfile → updateUser + pushCloudProfile + completeProfileSetup', display: 'userDataFlow.commitUserProfile', screens: ['Launch'] },
    { action: 'Set / change public User ID', dbMethod: 'commitUserProfile(publicUserId) + ensurePublicUserIdFree', display: 'userDataFlow; 7-day cooldown in settings', screens: ['Launch', 'Profile settings'] },
  ...
  user: [
    ...
    // Dual-lane profile: Supabase primary + Firebase mirror under linked uid (same public User ID).
    { action: 'Edit profile', dbMethod: 'commitUserProfile → pushCloudProfile (supabase + firebase mirror)', display: 'findUserById / targetUser; syncs embedded refs', screens: ['Profile'] },
  ...
  comment: [
    ...
    { action: 'Reply to reel comment', dbMethod: 'addReelCommentReply(..., buildCommentPayload(...))', display: 'resolveCommentAuthor', screens: ['Reels'] },
    { action: 'Like reel comment', dbMethod: 'likeReelComment(reelId, commentId, userId)', display: 'resolveCommentAuthor + comment.likes', screens: ['Reels'] },
  ],
  story: [
