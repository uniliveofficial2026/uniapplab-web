...
export const EMPTY_YOUTUBE_SEARCH_FILTERS: YoutubeSearchFilters = {};
...
export function countYoutubeSearchFilters(filters: YoutubeSearchFilters): number {
  let count = 0;
  if (filters.type && filters.type !== 'video') count += 1;
  if (filters.order && filters.order !== 'relevance') count += 1;
  if (filters.uploadDate && filters.uploadDate !== 'any') count += 1;
  if (filters.videoDuration && filters.videoDuration !== 'any') count += 1;
  if (filters.videoDefinition && filters.videoDefinition !== 'any') count += 1;
  if (filters.videoDimension && filters.videoDimension !== 'any') count += 1;
  if (filters.videoCaption && filters.videoCaption !== 'any') count += 1;
  if (filters.videoLicense && filters.videoLicense !== 'any') count += 1;
  if (filters.eventType && filters.eventType !== 'any') count += 1;
  if (filters.safeSearch && filters.safeSearch !== 'moderate') count += 1;
  if (filters.regionCode) count += 1;
  if (filters.relevanceLanguage) count += 1;
...
/** Client → `/api/youtube/search` query (no API key). */
export function youtubeSearchFiltersToQuery(
...
/** Upstream YouTube Data API v3 search.list URL query (caller supplies key). */
