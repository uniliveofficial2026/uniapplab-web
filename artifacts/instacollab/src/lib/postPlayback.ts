...
export function postPlaybackId(
  postId: string,
  surface: 'soundtrack' | 'text-audio' | 'video' | 'video-fs' | 'carousel-audio' | 'carousel-audio-fs'
): string {
...
export function resetPostPlayback(postId: string) {
  const surfaces = ['soundtrack', 'text-audio', 'video', 'video-fs', 'carousel-audio', 'carousel-audio-fs'] as const;
