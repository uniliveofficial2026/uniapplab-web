...
export type CreatorXpBreakdownLine = {
  key: string;
  label: string;
  detail: string;
  xp: number;
...
export function buildCreatorXpBreakdown(
  activity: CreatorActivityStats
): CreatorXpBreakdownLine[] {
  const w = CREATOR_XP_WEIGHTS;
  const lines: CreatorXpBreakdownLine[] = [
    { key: 'base', label: 'Creator base', detail: 'Starting bonus', xp: w.base },
    {
      key: 'posts',
      label: 'Posts',
      detail: `${activity.postCount.toLocaleString()} × ${w.post.toLocaleString()} XP`,
      xp: activity.postCount * w.post,
    },
    {
      key: 'reels',
      label: 'Reels',
      detail: `${activity.reelCount.toLocaleString()} × ${w.reel.toLocaleString()} XP`,
      xp: activity.reelCount * w.reel,
    },
    {
      key: 'likes',
      label: 'Likes received',
      detail: `${activity.likesReceived.toLocaleString()} × ${w.likeReceived} XP`,
      xp: activity.likesReceived * w.likeReceived,
    },
    {
      key: 'followers',
      label: 'Followers',
      detail: `${activity.followers.toLocaleString()} × ${w.follower} XP`,
      xp: activity.followers * w.follower,
    },
    {
      key: 'stories',
      label: 'Story segments',
      detail: `${activity.storySegmentCount.toLocaleString()} × ${w.storySegment} XP`,
      xp: activity.storySegmentCount * w.storySegment,
    },
    {
      key: 'visits',
      label: 'Profile visits',
      detail: `${activity.profileVisitCount.toLocaleString()} × ${w.profileVisit} XP`,
      xp: activity.profileVisitCount * w.profileVisit,
    },
  ];

  if (activity.hasActivePremium) {
    lines.push({
      key: 'premium',
      label: 'Profile premium',
      detail: 'Active premium bonus',
      xp: w.premiumBonus,
    });
  }
