...
import type { CommentsLayer, StoryViewScope } from '../layers';
...
export function WithComments<T extends Constructor<DbCoreBacked>>(Base: T): MixinCtor<T, CommentsLayer> {
  return class extends Base {
    ...
    private findReelCommentById(
      reelId: string,
      commentId: string,
      comments = this.asLocalDB().reelComments[reelId] ?? []
    ): CommentLike | null {
      for (const comment of comments) {
        if (comment?.id === commentId) return comment;
        const replies = Array.isArray(comment?.replies) ? comment.replies : [];
    ...
    getUserStorySegments(userId: string, scope: StoryViewScope = 'feed'): StoryDraftMedia[] {
      return scope === 'profile'
        ? this.asLocalDB().getProfileStorySegments(userId)
        : this.asLocalDB().getFeedStorySegments(userId);
    }
