...
function rowFromSnap(id: string, data: Record<string, unknown>): PartyRoomRow {
  return {
    ...
    room_key_hash: (data.room_key_hash as string | null) ?? null,
    seat_join_mode: String(data.seat_join_mode ?? 'free'),
    who_can_be_seated: String(data.who_can_be_seated ?? 'Anyone'),
