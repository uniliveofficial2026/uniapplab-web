...
import { resolveAppUserIdForDeviceAccount } from '../../lib/auth/storedAccountSessions';
...
export function AccountSwitcherModal({
  ...
  const isAccountActive = (acc: StoredDeviceAccount) => {
    if (!activeUid) return false;
    if (acc.uid === activeUid) return true;
    const appUserId = resolveAppUserIdForDeviceAccount(acc.uid);
    if (appUserId === activeUid) return true;
    if (activeUid && resolveAppUserIdForDeviceAccount(activeUid) === acc.uid) return true;
    return false;
  };

  const requestAccountSwitch = (acc: StoredDeviceAccount) => {
    if (isAccountActive(acc) || switchingUid) return;
  ...
  useEffect(() => {
    ...
    void Promise.resolve(onRefreshAccounts()).catch(() => {
      /* keep cached list */
    });
  ...
  const sortedAccounts = useMemo(() => {
    return [...displayAccounts].sort((a, b) => {
      if (isAccountActive(a)) return -1;
      if (isAccountActive(b)) return 1;
