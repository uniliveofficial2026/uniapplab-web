...
export function FollowListModal({
  ...
  const members = useMemo(
    ...
    [db, profileUserId, mode, db.users],
  );
  ...
  const openProfile = (userId: string) => {
    onClose();
    openUserProfileSurface({ userId, closeRoomFlow: true });
  };
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <ProfileNameLines
                        user={user}
                        primaryClassName="w-full truncate text-sm font-bold leading-tight flex items-center gap-1.5 min-w-0"
                        secondaryClassName="w-full truncate text-sm leading-tight text-foreground/65"
                        premiumBadge={<ProfilePremiumBadgeForUser user={user} size="sm" />}
                      />
                    </button>
                    {!isSelf ? (
                      <div className="relative flex shrink-0 items-center gap-1">
                        <button
                          type="button"
                          onClick={() => handleFollowToggle(user.id, getProfileMentionLabel(user))}
                        ...
                        <button
                          type="button"
                          aria-label={`More actions for ${getProfileMentionLabel(user)}`}
                          ...
                          <>
                            ...
                            >
                              <button
                                ...
                                onClick={() => handleBlockUser(user.id, getProfileMentionLabel(user))}
                              >
                                Block @{getProfileMentionLabel(user)}
                              </button>
                            </div>
                          </>
                        ) : null}
