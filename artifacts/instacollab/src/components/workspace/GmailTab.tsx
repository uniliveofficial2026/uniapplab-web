...
export function GmailTab() {
  ...
  const handleToggleStar = async (msg: GmailMessage, e: React.MouseEvent) => {
    ...
    if (googleAccessToken) {
      try {
        const res = await fetch(`https://www.googleapis.com/gmail/v1/users/me/messages/${msg.id}/modify`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${googleAccessToken}`,
            'Content-Type': 'application/json'
        ...
        if (!res.ok) throw new Error('Star update rejected');
      } catch (err) {
          ...
          {loading ? (
            ...
            messages.map((msg) => (
              <div 
                ...
                className={`group flex gap-3 p-3.5 hover:bg-secondary/25 cursor-pointer transition-colors relative ${msg.unread ? 'bg-primary/5 border-l-2 border-primary' : ''} ${selectedMsg?.id === msg.id ? 'bg-secondary/20' : ''}`}
              >
                <button 
      ...
      {selectedMsg && (
        <div className="lg:col-span-7 border border-border bg-card rounded-2xl shadow-sm flex flex-col overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          <div className="p-4 border-t border-border bg-secondary/5 flex gap-2">
            <button 
              onClick={() => {
                const emailMatch = selectedMsg.from.match(/<([^>]+)>/);
                const plainEmail = selectedMsg.from.includes('@') ? selectedMsg.from.trim() : '';
                setToInput(emailMatch?.[1] || plainEmail);
