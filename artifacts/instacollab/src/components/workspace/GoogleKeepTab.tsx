...
export function GoogleKeepTab() {
  const db = useDB();
  const storageKey = `google_keep_notes_${db.currentUser?.id ?? 'guest'}`;
  ...
  // Read notes from per-user storage on mount
  useEffect(() => {
    ...
    try {
      ...
      if (savedStr) {
        ...
        if (Array.isArray(parsed)) {
          savedNotes = parsed.filter(
            (note): note is KeepItem =>
              note &&
              typeof note === 'object' &&
  ...
  const filteredNotes = notes.filter(note => {
    ...
    return matchesSearch && note.archived === showArchived;
  });
