...
export function GoogleContactsTab() {
  ...
  }, [contactsStorageKey, googleAccessToken]);
  ...
  const handleToggleStar = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setContacts(prev => {
      const next = prev.map(c => c.id === id ? { ...c, starred: !c.starred } : c);
      persistLocalContacts(next);
      return next;
    });
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((c) => (
            ...
            >
              ...
              {/* Float action row */}
              <div className="mt-4 pt-3 border-t border-border/40 flex justify-end gap-1 relative z-10 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all">
                <button 
