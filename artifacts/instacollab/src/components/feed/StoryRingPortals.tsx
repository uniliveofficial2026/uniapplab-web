...
import { X, Heart, Volume2, VolumeX, Play, Pause, Maximize2, ArrowLeft } from 'lucide-react';
...
import { openProfilePreview, handleAvatarError, handleMediaError, formatMentionsAndTags, resolveEditorTextColorClass, resolveOverlayTextStyle, formatContentDateTime, contentTimestampIso, resolveAvatarSrc } from '../../lib/utils';
import { getStorySegmentPreviewUrl } from '../../lib/storyPreview';
...
export type StoryRingPortalsProps = {
  ...
  handoffToAdjacentStory: (targetId: string) => void;
  messageText: string;
...
export function StoryRingPortals(props: StoryRingPortalsProps) {
  ...
  const onViewerTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (deltaX < 0 && nextUserId) {
      handoffToAdjacentStory(nextUserId);
      return;
    }
    if (deltaX > 0 && prevUserId) {
  ...
  const storyHeaderDisplayName = getProfileDisplayName(storyUser);
  const storyHeaderMeta = [
    shouldShowProfileHandle(storyUser) ? formatProfileHandle(storyUser) : null,
    currentSegment?.createdAt
      ? formatContentDateTime(currentSegment.createdAt)
      : null,
  ]
    .filter(Boolean)
    .join(' · ');
            ...
            <AnimatePresence>
              {showStoryEmpty && (
                <div id="story-empty-modal" className={STORY_OVERLAY_SCRIM}>
                  ...
                  >
                    <StoryEmptyIntro
                      username={storyHeaderDisplayName}
                ...
                <div id="story-create-modal-root" className={STORY_CREATE_OVERLAY_SCRIM}>
                  ...
                  >
                    ...
                    <StoryCreatorFlow
                      ...
                      displayName={storyHeaderDisplayName}
                      avatarUrl={storyUser.avatarUrl}
                      layout="embedded"
            ...
            <AnimatePresence>
              {showStory && (
                ...
                <div className="absolute inset-0 z-0">
                  <img src={resolveAvatarSrc(storyUser.avatarUrl)} alt="Story" className="absolute inset-0 w-full h-full object-cover blur-3xl opacity-30" onError={handleAvatarError} />
                  {!currentSegment ? (
                ...
                {/* Progress Bar */}
                <div className="relative z-10 w-full shrink-0 pt-2 px-2 flex gap-1 pb-2">
                ...
                {/* Header */}
                <div className="relative z-10 shrink-0 px-4 pb-3 flex items-center justify-between gap-2 pointer-events-none bg-gradient-to-b from-black/55 via-black/25 to-transparent">
                   <div 
                     className="flex min-w-0 flex-1 items-center gap-3 pointer-events-auto cursor-pointer"
                     onClick={(e) => {
                       e.stopPropagation();
                       closeStoryViewer();
                   ...
                   >
                     <div className="relative size-10 shrink-0 rounded-full overflow-hidden border-2 border-white/90 bg-zinc-950 shadow-[0_2px_10px_rgba(0,0,0,0.45)] ring-1 ring-black/20">
                       <img
                         src={resolveAvatarSrc(storyUser.avatarUrl)}
                         alt={storyHeaderDisplayName}
                         className="block size-full object-cover object-center"
                         onError={handleAvatarError}
                       />
                     </div>
                     <div className="flex min-w-0 flex-col gap-0.5">
                       <span className="truncate text-sm font-bold text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)]">
                         {storyHeaderDisplayName}
                       </span>
                       {storyHeaderMeta ? (
                         <time
                           dateTime={
                             currentSegment?.createdAt
                               ? contentTimestampIso(currentSegment.createdAt)
                               : undefined
                           }
                           className="truncate text-[11px] font-medium text-white/80 drop-shadow-[0_1px_2px_rgba(0,0,0,0.75)]"
                         >
                           {storyHeaderMeta}
                         </time>
                       ) : null}
                ...
                <div className="flex-1 flex w-full relative z-10 mb-16">
                   <div
                     className="flex-1"
                     onPointerDown={onStoryHoldPointerDown}
                     onPointerUp={onStoryHoldPointerUp}
                     onPointerLeave={onStoryHoldPointerUp}
                     onPointerCancel={onStoryHoldPointerUp}
                ...
                <form onSubmit={sendMessage} className="relative z-10 p-4 pb-safe flex items-center gap-3">
                   ...
                   <button 
                     ...
                     className="group p-2 hover:bg-secondary rounded-full transition-colors text-foreground active:scale-95 transition-transform"
                     title="Share story"
                   >
                     <ShareIcon size="lg" />
                   </button>
