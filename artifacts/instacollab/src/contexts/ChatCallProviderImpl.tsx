...
export type PendingCallRef = React.MutableRefObject<{
  chatId: string;
  kind: 'audio' | 'video';
  mode: 'start' | 'join';
} | null>;

export function ChatCallProviderImpl({
  children,
  currentUserId,
  currentUserAvatarUrl,
  pendingCallRef,
  ...
  useEffect(() => {
    ...
    if (pending.mode === 'join') {
      void chatCall.joinGroupCall(pending.chatId, pending.kind);
    } else {
      chatCall.startCall(pending.chatId, pending.kind);
    }
  }, [chatCall, pendingCallRef]);
