...
async function ensureProfileRow(
  ...
  const { data: existing } = await service
    .from("profiles")
    .select("public_user_id, public_user_id_changed_at, profile_setup_complete")
    .eq("id", userId)
    .maybeSingle();

  const fromInput =
    input.publicUserId?.trim().toLowerCase().replace(/[^a-z0-9_]/g, "_").slice(0, 24) ||
    "";
  const publicUserId =
    (typeof existing?.public_user_id === "string" && existing.public_user_id.trim()) ||
    fromInput ||
    username;
  ...
  const { error } = await service.from("profiles").upsert(
    {
      ...
      profile_setup_complete: Boolean(
        input.profileSetupComplete || existing?.profile_setup_complete,
      ),
      public_user_id: publicUserId,
      public_user_id_changed_at:
        existing?.public_user_id_changed_at ?? new Date().toISOString(),
    },
    { onConflict: "id" },
  );
