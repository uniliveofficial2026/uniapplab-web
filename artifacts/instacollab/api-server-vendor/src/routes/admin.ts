...
/**
 * Unified Control Center moderation — prefers party room + host id so Firestore-only
 * live rooms still stop/ban even when streams rows are missing.
 */
router.post("/moderation/stop-live", async (req, res, next) => {
  try {
    const body = (req.body ?? {}) as {
      roomId?: string;
      streamId?: string;
    ...
    if (!roomId && !streamId && !hostUserId) {
      res.status(400).json({ error: "roomId_streamId_or_hostUserId_required" });
      return;
    }
    ...
    if (roomId) {
      const resolved = await resolvePartyRoom(roomId);
      const ownerId = resolved?.owner_id || hostUserId;
      if (resolved) {
        results.room = await endPartyRoomRecord(roomId, resolved.source);
      } else if (ownerId) {
        results.room = { id: roomId, status: "ended", owner_id: ownerId, source: "host_fallback" };
      }
      if (isLiveKitConfigured()) {
        await deleteLiveKitRoom(partyRoomName(roomId)).catch(() => false);
      }
    ...
    if (streamId) {
      ...
      if (stream) {
        const { data: stopped } = await sb
          ...
          .select("id, status, ended_at, user_id")
          .maybeSingle();
        results.stream = stopped;
        if (stream.user_id) await endActivePartyRoomsForOwner(String(stream.user_id));
      }
      if (isLiveKitConfigured()) {
        await deleteLiveKitRoom(streamRoomName(streamId)).catch(() => false);
      }
    ...
    if (!roomId && hostUserId) {
      results.endedPartyRoomIds = await endActivePartyRoomsForOwner(hostUserId);
      await getSupabaseService()
        .from("streams")
        .update({ status: "ended", ended_at: new Date().toISOString() })
...
router.post("/moderation/ban-host", async (req, res, next) => {
  try {
    const body = (req.body ?? {}) as {
      roomId?: string;
      streamId?: string;
    ...
    const roomId = typeof body.roomId === "string" ? body.roomId.trim() : "";
    const streamId = typeof body.streamId === "string" ? body.streamId.trim() : "";
    let hostUserId = typeof body.hostUserId === "string" ? body.hostUserId.trim() : "";
    const reason =
      typeof body.reason === "string" && body.reason.trim()
        ? body.reason.trim().slice(0, 500)
        : "Live room banned by platform admin";
    ...
    if (roomId) {
      const resolved = await resolvePartyRoom(roomId);
      if (resolved?.owner_id) hostUserId = resolved.owner_id;
      if (resolved) await endPartyRoomRecord(roomId, resolved.source);
      if (isLiveKitConfigured()) {
        await deleteLiveKitRoom(partyRoomName(roomId)).catch(() => false);
      }
    }
    ...
    if (streamId) {
      const { data: stream } = await getSupabaseService()
        ...
        .eq("id", streamId)
        .maybeSingle();
      if (stream?.user_id) hostUserId = String(stream.user_id);
      await getSupabaseService()
        .from("streams")
        .update({ status: "ended", ended_at: new Date().toISOString() })
        .eq("id", streamId);
    ...
    if (!hostUserId) {
      res.status(404).json({
        ...
        message: "Could not resolve host to ban. Pass hostUserId, roomId, or streamId.",
      });
      return;
    }

    await endActivePartyRoomsForOwner(hostUserId);
    await getSupabaseService()
      .from("streams")
      .update({ status: "ended", ended_at: new Date().toISOString() })
      .eq("user_id", hostUserId)
    ...
    const banned = await banProfile(hostUserId, reason);
    res.json({ ok: true, ban: banned, hostUserId, roomId: roomId || null, streamId: streamId || null });
  } catch (err) {
    next(err);
  }
});
