...
const prodDir = path.join(appRoot, '.local/greedy-tap-production/dist');
const outDir = path.join(appRoot, 'public/games/greedy-slot');

if (!fs.existsSync(prodDir)) {
  console.error('[sync-greedy-tap-static] Missing production build — run: pnpm greedy-tap:build');
  process.exit(1);
}
...
console.log(`[sync-greedy-tap-static] ✓ ${path.relative(appRoot, outDir)}`);
