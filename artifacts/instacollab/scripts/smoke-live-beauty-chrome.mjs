#!/usr/bin/env node
/**
 * Stage A smoke: Solo live → Solo live → Effects/beauty chrome.
...
async function main() {
  const hardDeadline = setTimeout(() => {
    console.error('[smoke-live-beauty-chrome] HARD_TIMEOUT');
    process.exit(2);
  }, 120_000);
  ...
  try {
    console.log(`[smoke-live-beauty-chrome] base=${base}`);
    console.log('[smoke-live-beauty-chrome] goto…');
    await page.goto(`${base}/home?launch=main&as=u1&force_demo=1`, {
      waitUntil: 'domcontentloaded',
      timeout: 45_000,
    });
    console.log('[smoke-live-beauty-chrome] dismiss…');
    if (!(await dismiss(page))) {
      evidence.skipped = 'shell_not_ready';
      evidence.ok = true;
      console.log('[smoke-live-beauty-chrome] SKIP (shell_not_ready)');
      console.log(JSON.stringify(evidence, null, 2));
      clearTimeout(hardDeadline);
      return;
    }

    console.log('[smoke-live-beauty-chrome] open create…');
    const hostReady = await waitForSelector(page, '[data-instant-room-host]', 12_000);
    console.log(`[smoke-live-beauty-chrome] host=${hostReady}`);
    await dispatchCreate(page);
    console.log('[smoke-live-beauty-chrome] dispatched');
    const entryReady = await waitForSelector(page, '[data-instant-room-entry]', 10_000);
    console.log(`[smoke-live-beauty-chrome] entry=${entryReady}`);
    ...
    console.log(`[smoke-live-beauty-chrome] createReady=${createReady}`);
    if (!createReady) {
      evidence.skipped = 'create_room_not_hydrated';
      evidence.ok = true;
      console.log('[smoke-live-beauty-chrome] SKIP (create_room_not_hydrated)');
      console.log(JSON.stringify(evidence, null, 2));
      clearTimeout(hardDeadline);
      return;
    }
    ...
    if (!evidence.liveRoom) {
      ...
      evidence.ok = true;
      evidence.screenshot = path.join(OUT_DIR, `live-beauty-chrome-${stamp}.png`);
      await page.screenshot({ path: evidence.screenshot }).catch(() => {});
      console.log('[smoke-live-beauty-chrome] SKIP (go_live_requires_stable_host_session)');
      console.log(JSON.stringify(evidence, null, 2));
      clearTimeout(hardDeadline);
      return;
    }

    evidence.beautyChrome = await page.evaluate(() => {
      const effects = Array.from(document.querySelectorAll('button')).some((b) =>
        /effects|beauty|makeup|filter/i.test(`${b.getAttribute('aria-label')||''} ${b.textContent||''}`),
      );
      return effects;
    });
    if (evidence.beautyChrome) {
      await page.evaluate(() => {
        const btn = Array.from(document.querySelectorAll('button')).find((b) =>
          /effects|beauty/i.test(`${b.getAttribute('aria-label')||''} ${b.textContent||''}`),
        );
        btn?.click();
      }).catch(()=>{});
      await page.waitForTimeout(700);
      evidence.beautyPanel = await page.evaluate(() =>
        /Beauty|Makeup|Filter|Effects|Stickers/i.test(document.body.innerText||'') ||
        !!document.querySelector('[data-ui-id*="beauty"], [data-ui-id*="effect"]'),
      );
    }
    evidence.ok = Boolean(evidence.beautyChrome && evidence.beautyPanel);
    if (!evidence.beautyChrome) evidence.blocker = 'beauty_effects_control_missing';
    else if (!evidence.beautyPanel) evidence.blocker = 'beauty_panel_missing_after_open';
    evidence.screenshot = path.join(OUT_DIR, `live-beauty-chrome-${stamp}.png`);
    await page.screenshot({ path: evidence.screenshot }).catch(() => {});
    console.log(`[smoke-live-beauty-chrome] ${evidence.ok ? 'PASS' : 'FAIL'}`);
    console.log(JSON.stringify(evidence, null, 2));
...
main().catch((err) => {
  console.error('[smoke-live-beauty-chrome] FATAL', err);
  process.exit(1);
});
