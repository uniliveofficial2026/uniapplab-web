#!/usr/bin/env node
/**
 * Pixel measurement + artwork audit from approved V15 MASTER reference JPEGs.
...
const refDir = path.join(repoRoot, 'UniLives-Final-Approved-UIUX-Production-Cursor-v15/reference-approved/MASTER/live-tools');
const assetDir = path.join(root, 'public/live-tools-v14');
const outDocs = path.join(root, 'docs/v15-visual-spec');
const cropDir = path.join(root, '.local-dev/v15-parity/reference-crops');
...
async function main() {
  ...
  fs.mkdirSync(path.join(root, '.local-dev/v15-parity'), { recursive: true });
  ...
  const auditPath = path.join(outDocs, 'artwork-audit-v15.json');
  const unique = [];
  const seen = new Set();
