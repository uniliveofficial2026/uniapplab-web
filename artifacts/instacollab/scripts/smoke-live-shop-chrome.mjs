#!/usr/bin/env node
/**
 * Stage A smoke: Solo live → Shop Live → commerce chrome.
...
async function dispatchCreate(page) {
  ...
  await page.evaluate((roomName) => {
    try {
      sessionStorage.setItem(
        'uni.createRoom.hint',
        JSON.stringify({ roomName, mode: 'Commerce-Live' }),
      );
    } catch {
      /* ignore */
    }
  ...
  }, 'StageA Shop Live Smoke');
}

async function main() {
  const hardDeadline = setTimeout(() => {
    console.error('[smoke-live-shop-chrome] HARD_TIMEOUT');
    process.exit(2);
  }, 120_000);
  ...
  try {
    console.log(`[smoke-live-shop-chrome] base=${base}`);
    console.log('[smoke-live-shop-chrome] goto…');
    await page.goto(`${base}/home?launch=main&as=u1&force_demo=1`, {
      waitUntil: 'domcontentloaded',
      timeout: 45_000,
    });
    console.log('[smoke-live-shop-chrome] dismiss…');
    if (!(await dismiss(page))) {
      evidence.skipped = 'shell_not_ready';
      evidence.ok = true;
      console.log('[smoke-live-shop-chrome] SKIP (shell_not_ready)');
      console.log(JSON.stringify(evidence, null, 2));
      clearTimeout(hardDeadline);
      return;
    }

    console.log('[smoke-live-shop-chrome] open create…');
    const hostReady = await waitForSelector(page, '[data-instant-room-host]', 12_000);
    console.log(`[smoke-live-shop-chrome] host=${hostReady}`);
    await dispatchCreate(page);
    console.log('[smoke-live-shop-chrome] dispatched');
    const entryReady = await waitForSelector(page, '[data-instant-room-entry]', 10_000);
    console.log(`[smoke-live-shop-chrome] entry=${entryReady}`);
    ...
    console.log(`[smoke-live-shop-chrome] createReady=${createReady}`);
    if (!createReady) {
      evidence.skipped = 'create_room_not_hydrated';
      evidence.ok = true;
      console.log('[smoke-live-shop-chrome] SKIP (create_room_not_hydrated)');
      console.log(JSON.stringify(evidence, null, 2));
      clearTimeout(hardDeadline);
      return;
    }
    ...
    await page.evaluate(() => {
      const shop = Array.from(document.querySelectorAll('button')).find((b) =>
        /^\s*Shop\s*$/i.test((b.textContent || '').trim()),
      );
      shop?.click();
    }).catch(() => {});
    ...
    if (!evidence.liveRoom) {
      ...
      evidence.ok = true;
      evidence.screenshot = path.join(OUT_DIR, `live-shop-chrome-${stamp}.png`);
      await page.screenshot({ path: evidence.screenshot }).catch(() => {});
      console.log('[smoke-live-shop-chrome] SKIP (go_live_requires_stable_host_session)');
      console.log(JSON.stringify(evidence, null, 2));
      clearTimeout(hardDeadline);
      return;
    }

    evidence.shopChrome = await page.evaluate(() => {
      const bag = Array.from(document.querySelectorAll('button')).some((b) =>
        /shop|product|bag|cart|commerce|pin/i.test(`${b.getAttribute('aria-label')||''} ${b.textContent||''}`),
      );
      const text = /Shop|Product|Pin|Order|Bag/i.test(document.body.innerText);
      return bag || text;
    });
    evidence.ok = evidence.shopChrome;
    if (!evidence.shopChrome) evidence.blocker = 'shop_live_commerce_chrome_missing';
    evidence.screenshot = path.join(OUT_DIR, `live-shop-chrome-${stamp}.png`);
    await page.screenshot({ path: evidence.screenshot }).catch(() => {});
    console.log(`[smoke-live-shop-chrome] ${evidence.ok ? 'PASS' : 'FAIL'}`);
    console.log(JSON.stringify(evidence, null, 2));
...
main().catch((err) => {
  console.error('[smoke-live-shop-chrome] FATAL', err);
  process.exit(1);
});
